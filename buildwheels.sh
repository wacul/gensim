...
# Compile wheel
"/opt/python/cp${pythonversion}-cp${pythonversion}${packaging}/bin/pip" wheel -v gensim==3.8.3 -w wheelhouse/

# Bundle external shared libraries into the wheel
auditwheel repair wheelhouse/gensim-*-cp${pythonversion}-cp${pythonversion}-linux_aarch64.whl -w /io/wheelhouse/
